...
/***/ function(module, exports, __webpack_require__) {
	...
	var pinwheel = document.getElementById('pinwheel');var kenny = document.getElementById('kenny');var beatState = 0;var bounce = _gsap.TweenMax.to("#face", 0.05, { scale: 1.05, svgOrigin: "400 300", ease: _gsap.Elastic.easeInOut, repeat: 1, paused: true, yoyo: true }); /** */function kick() {bounce.play(0);if (beatState) {pinwheel.style.fill = "#fff";wheel.style.fill = "#000";beatState = 0;} else {pinwheel.style.fill = "#000";wheel.style.fill = "#fff";beatState = 1;}} /* audio */ /** @type {HTMLAudioElement} */var audio; /** load song
...
/***/ function(module, exports, __webpack_require__) {
	...
	getEvents = getEvents; /* eslint-env browser */ /** @type {Object[]} */var labels = __webpack_require__(7).labels; /** @type {Object[]} */var eventList = []; /** @return {Object[]} An array of events, sorted in ascending order by time.*/function getEvents() {
	  // for each label, convert it to be used as an event
	  labels.forEach(function (label, i, array) {
	    if (label.end) {
	      // duration label, split into _open and _close events
	      eventList.push({
	        name: label.name + "_open",
	        time: label.start });
	
	      eventList.push({
	        name: label.name + "_close",
	        time: label.end });
	
	    } else {
	      // point label, single event
	      eventList.push({
	        name: label.name,
	        time: label.start });
...
/***/ function(module, exports) {
	...
	startScheduler = startScheduler; /* eslint-env browser */var audioEventStream = document.createElement(null); /** @type {AudioContext} */var ctx; /** @type {Object[]} */var events; /** @type {Number} duration to look ahead in seconds*/var lookAheadDuration = 0.30;var safety = 0.1;var index = 0; /** */function lookAhead() {var currentTime = ctx.currentTime;var latest = currentTime + lookAheadDuration; // if the next event is in the period we're looking at
	  ...
	  audioEventStream.dispatchEvent(new CustomEvent(event.name, { detail: { time: event.time } }));} /** Start the event scheduler.
	                                                                                                   ...
	                                                                                                   *  @param {Object[]} eventList the list of events to schedule
	                                                                                                   *  @param {EventTarget} eventTarget the EventTarget to dispatch events to
	                                                                                                  */function startScheduler(context, eventList, eventTarget) {audioEventStream = eventTarget;events = eventList;ctx = context;lookAhead();}
...
//# sourceMappingURL=zettai.js.map